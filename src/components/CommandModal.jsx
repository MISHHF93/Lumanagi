...
import { AdminLog } from "@/lib/entities";
import { useAppStore } from "@/store/useAppStore";
import { useAuth } from "@/contexts/AuthProvider";

export default function CommandModal({ userRole, open, onOpenChange }) {
  // Try to obtain user from Auth context first, fall back to app store user
  const { user: storeUser, fetchUser } = useAppStore((state) => ({
    user: state.user,
              ...
              >
                <div className="flex items-start justify-between">
                  <div className="flex items-start gap-3 flex-1">
                    ...
                    >
                      <command.icon
                        className={`w-5 h-5 ${command.danger ? "text-red-400" : "text-[#3B82F6]"}`}
                      />
                    </div>
